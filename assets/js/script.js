...
d3.json("map-widget/assets/json/topo_eer.json").then(data => {
  ...
  g.selectAll("path")
    ...
    if (info) {
      ...
      infoBox.html(`<strong>${name}</strong><p>No data available.</p>`);
    }

    event.stopPropagation();
  });

  // Render region labels as HTML using foreignObject
  ...
  // Set up zoom and pan with constraints
